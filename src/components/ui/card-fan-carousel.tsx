...
const MAX_VISIBLE = 7;
const HALF = 3;const FAN_POSITIONS = [
  { rot: -21, scale: 0.7756, x: -42, y: 7.3, zIndex: 1 },
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-hidden">
      <div className="flex items-center justify-center w-full max-w-[100rem]">
        <div
          ref={containerRef}
          className="flex relative justify-center items-center w-full max-w-[96rem]"
        >
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a
                ...
                href={card.linkUrl}
                target={
                  card.linkUrl.startsWith("http") ? "_blank" : "_self"
                }
      ...
      {/* Current card label */}
      {cards[centerIndex]?.alt && (
        <div className="mt-4 sm:mt-6 text-center z-30">
          <p className="font-mono text-sm sm:text-base uppercase tracking-[0.15em] text-text-secondary font-medium">
            {cards[centerIndex].alt}
          </p>
        </div>
      )}
            ...
            {cards.map((_, i) => (
              <span
                ...
                className={`hidden sm:block w-2 h-2 rounded-full transition-all duration-300 ${
                  i === centerIndex
                    ? "bg-accent-primary scale-[1.3]"
                    : "bg-white/15"
                }`
              }
              />
            ))}
            <span className="sm:hidden min-w-[40px] text-center text-xs font-mono text-white/50">
